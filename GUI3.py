...
def set_mode(selected_mode):
    ...
    # Toggle frame interval slider visibility based on mode
    if mode == "video":
...
def reset_gui():
    # Reset preview and progress bar for the next detection
...
# Frame interval slider for video only
...
confidence_slider = tk.Scale(confidence_frame, from_=0.1, to=1.0, resolution=0.1, orient="horizontal", label="Select Confidence Threshold")
...
# Preview area
preview_frame = tk.LabelFrame(app, text="Preview", padx=10, pady=10)
